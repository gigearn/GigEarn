...
// Internal User Protected Route (Super Admin or Verifier)
export const InternalRoute = ({ children }) => {
  const { isAuthenticated, isInternalUser, isLoading } = useAuth();
  const location = useLocation();
...
export const RoleAccess = ({ children, allowedRoles, fallback = null }) => {
  const { user } = useAuth();
